...
typename __select_2nd<decltype((std::declval<_Tp>() = std::declval<_Arg>())), true_type>::type
#ifndef _LIBCPP_HAS_NO_RVALUE_REFERENCES
...
template <class _Tp, class ..._Args>
typename __select_2nd<decltype(std::move(_Tp(std::declval<_Args>()...))), true_type>::type
...
template <class _Tp, class _A0>
decltype((_Tp(std::declval<_A0>()), true_type()))
...
decltype((_Tp(std::declval<_A0>(), std::declval<_A1>()), true_type()))
...
struct __libcpp_is_nothrow_assignable<true, _Tp, _Arg>
    : public integral_constant<bool, noexcept(std::declval<_Tp>() = std::declval<_Arg>()) >
{
};
...
iter_swap(_ForwardIterator1 __a, _ForwardIterator2 __b)
    //                                  _NOEXCEPT_(_NOEXCEPT_(swap(*__a, *__b)))
               _NOEXCEPT_(_NOEXCEPT_(swap(*std::declval<_ForwardIterator1>(),
                                          *std::declval<_ForwardIterator2>())))
{
    swap(*__a, *__b);
}
...
struct __is_nothrow_swappable_imp
    : public integral_constant<bool, noexcept(swap(std::declval<_Tp&>(),
                                                   std::declval<_Tp&>()))>
{
};
...
struct __has_operator_addressof_member_imp
{
    template <class _Up>
        static auto __test(int)
            -> typename __select_2nd<decltype(std::declval<_Up>().operator&()), true_type>::type;
    template <class>
...
struct __has_operator_addressof_free_imp
{
    template <class _Up>
        static auto __test(int)
            -> typename __select_2nd<decltype(operator&(std::declval<_Up>())), true_type>::type;
    template <class>
        static auto __test(long) -> false_type;
